...
class CardUtilsTest(BaseUnitTest):
    ...
    def test_gen_cards(self):
        self.eq([Card(rank=13, suit=4), Card(rank=11, suit=2)], U.gen_cards(['Ac', 'Qd']))
        self.eq([Card(rank=8, suit=8), Card(rank=9, suit=4)], U.gen_cards(['9H', 'Tc']))
        self.eq([Card(rank=10, suit=16), Card(rank=12, suit=2)], U.gen_cards(['Js', 'Kd']))
    ...
    def test_montecarlo_simulation(self):
        ...
        mock_return = [U.gen_cards(a) for a in [['7d'], ['Kd', 'Kh', '8h', 'As']]]
        ...
        mock_return = [U.gen_cards(a) for a in [['7s'], ['Kd', 'Kh', '8h', 'As']]]
        with patch('pypokerengine.utils.card_utils._pick_unused_card', side_effect=mock_return):
