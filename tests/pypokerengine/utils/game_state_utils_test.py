...
class GameStateUtils(BaseUnitTest):
    ...
    def test_attach_hole_card(self):
        ...
        hole1, hole2 = [to_card(c) for c in ['As', 'Ad']], [to_card(c) for c in ['Kh', '2c']]
    ...
    def test_restore_game_state_two_players_game(self):
        ...
        self.eq(['5d', '9d', '6h', 'Kc'], [str(card) for card in table.get_community_card()])
        self._assert_deck(table.deck, [Card.from_str(s) for s in ['5d', '9d', '6h', 'Kc']])
    ...
    def test_restore_game_state_three_players_game(self):
        ...
        self.eq(['Jh', '8c', '2d', '4h'], [str(card) for card in table.get_community_card()])
        self._assert_deck(table.deck, [Card.from_str(s) for s in ['Jh', '8c', '2d', '4h']])
