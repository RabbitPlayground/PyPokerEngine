...
class ActionCheckerTest(BaseUnitTest):
    ...
    def test_correct_illegal_call(self):
        players = self.__setup_clean_players()
        self.error(lambda: ActionChecker.correct_action(players, 0, 2.5, 'call', 10))
    ...
    def test_correct_illegal_raise(self):
        players = self.__setup_clean_players()
        self.error(lambda: ActionChecker.correct_action(players, 0, 2.5, 'raise', 101))
    ...
    def test_legal_actions(self):
        ...
        self.eq({'action': 'call', 'amount': 10, 'paid': 5}, legal_actions[1])
        self.eq({'action': 'raise', 'amount': {'min': 15, 'max': 100}, 'paid': 5}, legal_actions[2])
    ...
    def test_legal_actions_when_short_of_money(self):
        ...
        self.eq({'action': 'fold', 'amount': 0}, legal_actions[0])
        self.eq({'action': 'call', 'amount': 10, 'paid': 5}, legal_actions[1])
        self.eq({'action': 'raise', 'amount': {'min': 14, 'max': 14}, 'paid': 5}, legal_actions[2])
    ...
    def test_need_amount_after_ante(self):
        ...
        self.eq(('fold', 0), ActionChecker.correct_action(players, 2, 5, 'fold', 0))
