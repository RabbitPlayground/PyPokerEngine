...
class RoundManagerTest(BaseUnitTest):
    ...
    def test_state_after_forward_to_flop(self):
        ...
        def fetch_player(uuid):
            return [p for p in state['table'].seats.players if p.uuid == uuid][0]
    ...
    def test_everyone_agree_logic_regression(self):
        ...
        state, _ = RoundManager.apply_action(state, 'call', 30)
        state, _ = RoundManager.apply_action(state, 'raise', 50)
        state, _ = RoundManager.apply_action(state, 'call', 50)
    ...
    def test_add_amount_calculation_when_raise_on_ante(self):
        ...
        def pot_amount(state):
            return GameEvaluator.create_pot(state['table'].seats.players)[0]['amount']

        def stack_check(expected, state):
            return self.eq(expected, [p.stack for p in state['table'].seats.players])
        ...
        self.eq(40, raised_state['table'].seats.players[2].action_histories[-1]['paid'])
